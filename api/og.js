...
function data(req){
  ...
  return {
    ...
    group:groupName(q(req,'group',q(req,'stage',''))), date:q(req,'date',''), time:q(req,'time',q(req,'kickoff','')),
    status:q(req,'status', (q(req,'hg','')!=='' && q(req,'ag','')!=='') ? 'FT' : 'Upcoming'),
...
function html(req,d){
  const title=`${d.home} vs ${d.away} · World Cup 2026`, desc=[meta(d),d.venue].filter(Boolean).join(' • '), showMarket=d.homeSim||d.drawSim||d.awaySim||d.homePoly||d.awayPoly;

  // Venue + weather collapse into a single row (icon, venue link · city on the
  // left, temperature on the right) — mirrors the in-app match modal instead
  // of stacking them as two separate boxed sections.
  const venueRow = (d.venue || d.city || d.tempLine) ? `<div class="infoRow"><div class="left"><span class="ico">📍</span>${d.venue?`<a class="venueLink" href="${esc(maps(d.venue,d.city))}" target="_blank" rel="noopener">${esc(d.venue)}</a>`:`<span class="venueLink plain">Venue TBA</span>`}${d.city?`<span class="citySub"> · ${esc(d.city)}</span>`:''}</div>${d.tempLine?`<div class="wxRight">🌤️ ${esc(d.tempLine)}</div>`:''}</div>` : '';

  // Broadcast is one inline row — channels in gold, streaming options in
  // gray right after, same as the in-app card (no separate label/value/sub).
  const broadcastRow = d.broadcast ? `<div class="infoRow"><div class="left"><span class="ico">📺</span><span class="bcMain">${esc(d.broadcast)}</span>${d.streaming?`<span class="bcSub"> · ${esc(d.streaming)}</span>`:''}</div></div>` : '';

  const probSection = showMarket ? `<div class="probLabel">WIN PROBABILITY</div><div class="probRow"><div class="probBox home">${d.homeFlagUrl?`<img class="probFlag" src="${esc(d.homeFlagUrl)}" alt=""/>`:''}<div class="probName">${esc(d.home)}</div><div class="probPct">${esc(d.homeSim||d.homePoly||'—')}%</div></div><div class="probBox draw"><div class="probName">Draw</div><div class="probPct">${esc(d.drawSim||'—')}%</div></div><div class="probBox away">${d.awayFlagUrl?`<img class="probFlag" src="${esc(d.awayFlagUrl)}" alt=""/>`:''}<div class="probName">${esc(d.away)}</div><div class="probPct">${esc(d.awaySim||d.awayPoly||'—')}%</div></div></div>` : '';
  ...
  :root{color-scheme:dark;--green:#4ade80;--text:#eafff0;--blue:#60a5fa;--gold:#fbbf24}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 80% 0%,#0d2a18 0,#030905 42%,#010402 100%);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial,sans-serif;color:var(--text)}.wrap{max-width:760px;margin:0 auto;padding:18px 18px calc(22px + env(safe-area-inset-bottom))}.brand{font-weight:800;line-height:.98;margin:3px 0 12px}.brand small{display:block;letter-spacing:.32em;color:#72a981;font-size:10px;font-weight:700;margin-bottom:3px}.brand .wc{font-size:19px}.brand .yr{font-size:23px;color:#4ade80}.hero{border:1px solid rgba(74,222,128,.28);background:linear-gradient(135deg,#0b2415,#06130b);border-radius:23px;padding:14px 16px 15px;box-shadow:0 10px 28px rgba(0,0,0,.28);overflow:hidden}.groupLabel{text-align:center;color:#77f0a0;font-size:11px;font-weight:700;letter-spacing:.12em}.dateLine{text-align:center;color:#9fc5aa;font-size:12px;font-weight:500;margin-top:2px;margin-bottom:6px}.teams{display:grid;grid-template-columns:1fr auto 1fr;gap:8px;align-items:center;min-height:92px;margin-top:8px}.team{text-align:center}.flag{font-size:26px;line-height:1;margin-bottom:4px}.flag-img{width:38px;height:26px;object-fit:cover;border-radius:3px;margin:0 auto 4px;display:block;box-shadow:0 1px 3px rgba(0,0,0,.4)}.name{font-size:14px;font-weight:700}.vs{color:#45da78;font-size:20px;font-weight:800;text-align:center}.status{text-align:center;color:#6fa780;font-size:9px;font-weight:700;letter-spacing:.18em;margin-top:2px}.details{margin-top:10px;display:grid;gap:8px}.probLabel{text-align:center;font-size:11px;font-weight:700;letter-spacing:.12em;color:#6f9c80;margin:12px 0 8px}.probRow{display:flex;gap:8px;margin-bottom:10px}.probBox{flex:1;display:flex;flex-direction:column;align-items:center;border-radius:16px;padding:10px 6px;text-align:center}.probBox.home{background:rgba(74,222,128,.10);border:1px solid rgba(74,222,128,.4)}.probBox.draw{background:rgba(250,204,21,.10);border:1px solid rgba(250,204,21,.4)}.probBox.away{background:rgba(248,113,113,.10);border:1px solid rgba(248,113,113,.4)}.probFlag{width:16px;height:11px;border-radius:2px;object-fit:cover;margin-bottom:3px}.probName{font-size:11px;font-weight:700;color:#9fc5aa;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%}.probPct{font-size:18px;font-weight:800;margin-top:3px}.probBox.home .probPct{color:#4ade80}.probBox.draw .probPct{color:#facc15}.probBox.away .probPct{color:#f87171}.infoRow{display:flex;align-items:center;justify-content:space-between;gap:12px;border:1px solid rgba(74,222,128,.24);background:rgba(9,31,18,.85);border-radius:17px;padding:11px 14px}.infoRow .left{display:flex;align-items:center;gap:0;flex-wrap:wrap;min-width:0}.row{display:grid;grid-template-columns:28px 1fr 16px;gap:9px;align-items:center;border:1px solid rgba(74,222,128,.24);background:rgba(9,31,18,.85);border-radius:17px;padding:10px 12px;text-decoration:none;color:inherit}.ico{font-size:16px;margin-right:8px;flex-shrink:0}.venueLink{color:var(--blue);font-weight:700;font-size:14px;text-decoration:underline}.venueLink.plain{text-decoration:none;color:#9fc5aa}.citySub{color:#9fc5aa;font-weight:400;font-size:13px}.wxRight{font-weight:700;font-size:14px;color:#f1fff5;white-space:nowrap;flex-shrink:0}.bcMain{color:var(--gold);font-weight:700;font-size:14px}.bcSub{color:#9fc5aa;font-weight:400;font-size:13px}.label{font-size:10px;letter-spacing:.14em;font-weight:700;color:#80a98b;text-transform:uppercase;margin-bottom:3px}.value{font-size:15px;font-weight:700;color:var(--blue);line-height:1.15}.sub{font-size:12px;color:#9fc5aa;margin-top:2px;line-height:1.18}.actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:13px}.btn{border-radius:18px;padding:11px 10px;border:1px solid rgba(74,222,128,.34);background:#0b2415;color:#58e887;text-decoration:none;text-align:center;font-size:15px;font-weight:700}.btn.primary{background:linear-gradient(135deg,#4ade80,#22c55e);color:#031008;border:0}.install{text-align:center;color:#77a484;font-size:13px;line-height:1.32;margin-top:10px}@media(min-width:700px){.brand .wc{font-size:26px}.brand .yr{font-size:30px}.flag{font-size:36px}.flag-img{width:50px;height:34px}.name{font-size:17px}.vs{font-size:27px}.teams{min-height:110px}.groupLabel{font-size:13px}.dateLine{font-size:14px}.value{font-size:17px}.sub{font-size:14px}.venueLink,.wxRight,.bcMain{font-size:16px}.citySub,.bcSub{font-size:14px}.hero{padding:17px}.row{padding:14px}}
  </style></head><body><main class="wrap"><div class="brand"><small>FIFA</small><div class="wc">WORLD CUP</div><div class="yr">2026</div></div><section class="hero"><div class="groupLabel">${esc((d.group||'WORLD CUP 2026').toUpperCase())}</div>${(d.date||d.time)?`<div class="dateLine">${esc([d.date,d.time].filter(Boolean).join(' · '))}</div>`:''}<div class="teams"><div class="team">${d.homeFlagUrl?`<img class="flag-img" src="${esc(d.homeFlagUrl)}" alt=""/>`:`<div class="flag">🏳️</div>`}<div class="name">${esc(d.home)}</div></div><div><div class="vs">${esc(scoreOrVs(d))}</div><div class="status">${esc(String(d.status).toUpperCase())}</div></div><div class="team">${d.awayFlagUrl?`<img class="flag-img" src="${esc(d.awayFlagUrl)}" alt=""/>`:`<div class="flag">🏳️</div>`}<div class="name">${esc(d.away)}</div></div></div></section>${probSection}<div class="details">${venueRow}${broadcastRow}</div><div class="actions"><a class="btn primary" href="${esc(base(req))}">⚽ Open App</a><button class="btn" onclick="navigator.share?navigator.share({title:${JSON.stringify(title)},text:${JSON.stringify(desc)},url:location.href}):navigator.clipboard.writeText(location.href)">📤 Share</button></div><div class="install"><strong>Don’t have the app?</strong><br/>iPhone: Safari → Share ↑ → Add to Home Screen<br/>Android: Chrome → ⋯ → Add to Home Screen</div></main></body></html>`;
}
