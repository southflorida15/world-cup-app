    ...
    <div>
      ...
      <div ref={_ghRef} style={{position:"relative",top:0,left:"auto",transform:"none",width:"100%",maxWidth:700,zIndex:2,background:C.bg,borderBottom:`2px solid ${C.b2}`,boxShadow:"none",padding:"8px 13px",marginBottom:0}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <Pill active={view==="standings"} onClick={()=>setView("standings")} color={C.gold}>{tx("📊 Standings", "📊 Classificação")}</Pill>
          <Pill active={view==="matches"} onClick={()=>setView("matches")} color={C.gold}>{tx("📋 Matches", "📋 Jogos")}</Pill>
        </div>
      </div>
        ...
        <div>
          <Card style={{marginBottom:12}}>
            <div style={{padding:"8px 13px",borderBottom:`1px solid ${C.b1}`,background:C.s1,display:"flex",justifyContent:"space-between"}}>
              <span style={{fontWeight:700,color:C.green,fontSize:15}}>{tx("GROUP", "GRUPO")} {sel}</span>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                {liveCount>0 && <Badge color={C.green}>🔴 Live</Badge>}
                {lastFetch && <span style={{fontSize:10,color:C.dim}}>{tx("Updated", "Atualizado")} {lastFetch.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}</span>}
                <span style={{fontSize:10,color:C.dim}}>{tx("Tap for stats", "Toque para estatísticas")}</span>
              </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"22px 1fr 28px 28px 28px 28px 32px 32px",padding:"4px 10px",borderBottom:`1px solid ${C.b1}`,background:C.bg}}>
              {["#",tx("Team", "Time"),"P","W","D","L","SG",tx("Pts", "Pts")].map((h,i)=><div key={i} style={{fontSize:11,color:C.dim,fontWeight:700,textAlign:i>=2?"center":"left"}}>{h}</div>)}
            </div>
            {standings.map((row,i)=>(
              <div key={row.team} onClick={()=>onTeam(row.team)} style={{display:"grid",gridTemplateColumns:"22px 1fr 28px 28px 28px 28px 32px 32px",padding:"9px 10px",borderBottom:i<3?`1px solid ${C.b1}`:"none",cursor:"pointer",borderLeft:`3px solid ${qc(row.pos)}`,background:row.pos<=2?`${C.green}08`:row.pos===3?`${C.gold}08`:"transparent"}} onMouseEnter={e=>e.currentTarget.style.background=`${C.green}12`} onMouseLeave={e=>e.currentTarget.style.background=row.pos<=2?`${C.green}08`:row.pos===3?`${C.gold}08`:"transparent"}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  ...
                  <span style={{fontSize:14,color:C.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{displayTeamName(row.team, language)}</span>
                </div>
            ...
            <div style={{padding:"6px 12px",borderTop:`1px solid ${C.b1}`,display:"flex",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:4,fontSize:12,color:C.dim}}><div style={{width:9,height:9,background:C.green,borderRadius:2}}/>{tx("Top 2 Qualify", "Top 2 classificam")}</div>
              <div style={{display:"flex",alignItems:"center",gap:4,fontSize:12,color:C.dim}}><div style={{width:9,height:9,background:C.gold,borderRadius:2}}/>{tx("Best 3rd", "Melhores 3ºs")}</div>
            </div>
          </Card>
          <div style={{fontSize:11,color:C.dim,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>{tx("ENTER SCORES", "INSERIR PLACARES")}</div>
          <div style={{fontSize:11,color:C.dim,marginBottom:8,lineHeight:1.5}}>{tx("Scores fill in automatically as matches are played — live ones update in real time (🔴). Type your own score to play out a what if, then tap Reset to go back to the actual result.", "Os placares entram automaticamente conforme os jogos acontecem — jogos ao vivo atualizam em tempo real (🔴). Digite seu próprio placar para simular um cenário e toque em ↺ Reset para voltar ao resultado real.")}</div>
          {results.map(r=>{
            ...
            <div key={r.id} style={{display:"flex",alignItems:"center",gap:6,marginBottom:6,background:C.s1,border:`1px solid ${C.b1}`,borderRadius:10,padding:"7px 11px"}}>
              ...
              <span onClick={()=>fullMatch&&onMatchTap&&onMatchTap(fullMatch)} style={{fontSize:13,color:C.text,flex:1,textAlign:"right",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",cursor:onMatchTap?"pointer":"default"}}>{displayTeamName(r.home, language)}</span>
              ...
              {hasManualOverride && (
                <button onClick={()=>resetToActual(r.id)} title={tx("Reset to actual score", "Voltar ao placar real")} style={{fontSize:10,color:C.gold,background:`${C.gold}18`,border:`1px solid ${C.gold}44`,borderRadius:6,padding:"3px 6px",cursor:"pointer",fontWeight:700,flexShrink:0,whiteSpace:"nowrap"}}>{tx("↺ Reset", "↺ Reset")}</button>
              )}
              <span onClick={()=>fullMatch&&onMatchTap&&onMatchTap(fullMatch)} style={{fontSize:13,color:C.text,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",cursor:onMatchTap?"pointer":"default"}}>{displayTeamName(r.away, language)}</span>
        ...
        <Card key={r.id} style={{marginBottom:8,cursor:"pointer"}} onClick={()=>fullMatch&&onMatchTap&&onMatchTap(fullMatch)}>
          <div style={{padding:"11px 13px"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontWeight:700,color:C.text,flex:1,fontSize:14}}>{displayTeamName(r.home, language)}</span>
              ...
              <span style={{fontWeight:700,color:C.text,flex:1,textAlign:"right",fontSize:14}}>{displayTeamName(r.away, language)}</span>
