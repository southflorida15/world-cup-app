...
import { getBroadcastLabel, getBroadcastGroupsForMatch } from "./utils/broadcasts.js";
...
import { buildQualifiedThirdsFromSelectedTeams, buildThirdGroupsKey, ROUND_OF_16_TEMPLATE, QUARTER_FINAL_TEMPLATE, SEMI_FINAL_TEMPLATE, THIRD_PLACE_TEMPLATE, FINAL_TEMPLATE } from "./engine/fifa2026Bracket";
...
export const MATCHES = [
  ...
  {id:89, date:"Jul 4", time:"1PM ET",   home:"R16 M1",  away:"TBD",venue:"Houston Stadium, Houston",              group:"",stage:"Round of 16",tv:"FOX · Peacock · Telemundo"},
  {id:90, date:"Jul 4", time:"5PM ET",   home:"R16 M2",  away:"TBD",venue:"Philadelphia Stadium, Philadelphia",    group:"",stage:"Round of 16",tv:"FOX · Peacock · Telemundo"},
...
const statusLabel = (s,e,ex,language="en") => {
  ...
  const rawMinute = e ? (ex ? `${e}+${ex}` : `${e}`) : null;
  const minuteLabel = rawMinute ? formatDisplayMinute(rawMinute, language) : null;

  if(s==="1H"||s==="first_half"||s==="inprogress"||s==="LIVE") return minuteLabel || "LIVE";
  if(s==="HT"||s==="halftime") return "HT";
  if(s==="2H"||s==="second_half") return minuteLabel || "LIVE";
  if(s==="ET"||s==="extra_time"||u==="STATUS_EXTRA_TIME"||u==="STATUS_OVERTIME") return minuteLabel ? `ET ${minuteLabel}` : "ET";
  ...
  if(u==="AET"||u==="AFTER_EXTRA_TIME"||u==="STATUS_FINAL_AET"||u==="STATUS_END_OF_EXTRATIME") return "AET";
  if(u==="PEN"||u==="AFTER_PENALTIES"||u==="STATUS_FINAL_PEN"||u==="STATUS_FINAL_PENALTY"||u==="STATUS_FINAL_PENALTIES") return "Pens";
  return s;
};

function splitMinuteDisplay(label) {
  const str = String(label || "");
  const m = str.match(/^(.*?)\s+\((.*)\)$/);
  return m ? { primary: m[1], context: m[2] } : { primary: str, context: "" };
}

function MinuteDisplay({ raw, language="en" }) {
  const { primary, context } = splitMinuteDisplay(formatDisplayMinute(raw, language));
  return (
...
function eventMinuteRaw(ev) {
  if (!ev?.time) return "";
  if (ev.time.display) return ev.time.display;
  const elapsed = ev.time.elapsed ?? "";
  const extra = ev.time.extra ?? null;
  if (elapsed === "") return "";
  return extra != null && extra !== "" ? `${elapsed}+${extra}` : `${elapsed}`;
}

function eventPlayerDisplay(ev, language="en") {
  const name = ev?.player?.name || "";
  if (!name) return "";
  const pt = language === "pt-BR" || language === "pt";
  if (ev?.type === "Goal" && ev?.detail === "Own Goal") return `${name} ${pt ? "(contra)" : "(OG)"}`;
  if (ev?.type === "Goal" && ev?.detail === "Penalty") return `${name} ${pt ? "(PÊN.)" : "(PEN)"}`;
  return name;
}
  ...
  return (
    <div onClick={()=>onMatchTap&&onMatchTap(m)} style={{marginBottom:8,background:isKickingOff?countdownBg:live?`linear-gradient(135deg,${C.green}14,${C.s1})`:C.s1,border:`${isKickingOff?"3px":live?"3px":"1px"} solid ${isKickingOff?C.gold:live?C.green:finished&&winner?C.green:isFav?`${C.gold}66`:C.b2}`,borderRadius:12,overflow:"hidden",opacity:finished?0.8:1,cursor:onMatchTap?"pointer":"default",boxShadow:isKickingOff?`0 0 0 2px ${C.gold}44,0 4px 24px ${C.gold}33`:live?`0 0 0 1px ${C.green}44,0 4px 20px ${C.green}22`:finished&&winner?`0 0 0 1px ${C.green}22`:"none",transition:"border-color .3s,box-shadow .3s,background .3s"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"7px 13px",borderBottom:`1px solid ${C.b1}`,background:C.s2}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
          {live && <span style={{fontSize:10,fontWeight:700,color:C.green}}>🔴 {statusLabel(sc.status,sc.elapsed,sc.elapsedExtra,language)}</span>}
      ...
      <div onClick={e=>e.stopPropagation()} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 13px",borderTop:`1px solid ${C.b2}`,background:C.s2}}>
        <div style={{fontSize:11,color:finished?C.dim:C.mid,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1}}>
          <BroadcastBadges match={m} country={country} C={C} compact showConfidence />
        </div>
...
function getBroadcastCountrySafe() {
  try {
    const saved = localStorage.getItem("broadcastCountry");
    if (saved) return saved;
    const lang = navigator.language || (navigator.languages && navigator.languages[0]) || "en-US";
    return (lang.split("-").pop() || "US").toUpperCase();
  } catch {
    return "US";
  }
}
  ...
  setTimeout(() => {
    if (Notification.permission === "granted") {
      new Notification(`⚽ ${match.home} vs ${match.away} — Kick-off in ${minsBeforeKO} min!`, {
        body: `📍 ${match.venue}\n📺 ${getBroadcastLabel(match, getBroadcastCountrySafe()) || match.tv || ""}`,
        icon: "/favicon.ico",
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
          ...
          {hasScore ? (
            <div style={{textAlign:"center",minWidth:64}}>
              ...
              {statusLabel(sc.status,sc.elapsed,sc.elapsedExtra,language) && <div style={{fontSize:10,fontWeight:700,marginTop:2,color:live?C.green:C.dim}}>{live?"🔴 ":""}{statusLabel(sc.status,sc.elapsed,sc.elapsedExtra,language)}</div>}
            </div>
          ) : (
...
export function getResolvedTournamentMatches({ getScore=()=>null } = {}) {
  ...
  if (typeof ref === "string" && ref.startsWith("W")) {
    return winnerMap[Number(ref.slice(1))] || "TBD";
  }

  if (typeof ref === "string" && ref.startsWith("L")) {
    return loserMap[Number(ref.slice(1))] || "TBD";
  }
  ...
  template.forEach(t => {
    ...
    if (winner) {
      winnerMap[t.match] = winner;
      loserMap[t.match] = winner === home ? away : home;
    }

    dynamicById[t.match] = decorate(
      t.match,
      home,
      away,
      { 
        homeSlot:t.home, 
        awaySlot:t.away, 
        homeTag:refTag(t.home), 
        awayTag:refTag(t.away) 
      }
    );
  });
};
  ...
  buildRound(THIRD_PLACE_TEMPLATE);
  buildRound(FINAL_TEMPLATE);

console.log("WINNERS", winnerMap);
console.log("THIRD PLACE", dynamicById[THIRD_PLACE_TEMPLATE[0].match]);
        ...
        <div style={{maxHeight:360,overflowY:"auto",paddingRight:4}}>
          {rows.map((r, i) => (
            <div key={r.key} style={{display:"grid",gridTemplateColumns:"46px 1fr",gap:10,padding:"9px 0",borderBottom:i<rows.length-1?`1px solid ${C.b1}`:"none"}}>
              <div style={{fontSize:12,fontWeight:900,color:C.gold,textAlign:"right"}}><MinuteDisplay raw={r.time} language={language} /></div>
              <div>
...
function MatchEventsModal({ match, open, onClose, onAction, savedIds=new Set(), userPredHg, userPredAg }) {
  ...
  useEffect(() => {
    ...
    // Prevent the page behind the match modal from scrolling/bouncing.
    // This fixes the occasional screen flash while scrolling long match cards on mobile.
  ...
  const shareMatch = async () => {
    ...
    const broadcastGroups = getBroadcastGroupsForMatch(match, country, { tvLimit: 3, streamLimit: 2, includeRadio: false });
    const tvLabel = broadcastGroups.tv.map(b => b.name).filter(Boolean).join(" • ");
    const streamLabel = broadcastGroups.stream.map(b => b.name).filter(Boolean).join(" • ");
    // OG cards use separate fields so TV and streaming don't get duplicated or mixed.
    // No radio. No source names. No +1/+2.
    if (tvLabel) params.set("broadcast", `📺 ${tvLabel}`);
    if (streamLabel) params.set("streaming", `▶️ ${streamLabel}`);
    if (!finished && simOdds) {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,height:"100dvh",overflow:"hidden",background:"rgba(0,0,0,.85)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:"8px 3px",touchAction:"none"}}>
      ...
      <div ref={modalRef} onClick={e=>e.stopPropagation()}
        ...
        style={{background:C.bg,border:`1px solid ${C.b2}`,borderRadius:"18px 18px 18px 18px",width:"100%",maxHeight:"calc(100dvh - 50px)",overflowY:"auto",overscrollBehavior:"contain",WebkitOverflowScrolling:"touch",touchAction:"pan-y",paddingBottom:8,position:"relative",transform:"translateZ(0)",backfaceVisibility:"hidden",WebkitBackfaceVisibility:"hidden",contain:"paint"}}>
              ...
              <div style={{marginBottom:12}}>
                <>
                    ...
                    {!loading && filtered.length > 0 && filtered.map((ev,i)=>{
                      ...
                      const playerLabel = eventPlayerDisplay(ev, language);
                      return (
                        <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderBottom:`1px solid ${C.b1}`}}>
                          <div style={{flex:1,textAlign:"right"}}>
                            {isHome && ev.type!=="subst" && <span style={{fontSize:13,color:C.text,fontWeight:ev.type==="Goal"?700:400}}>{playerLabel}</span>}
                          ...
                          <div style={{display:"flex",flexDirection:"column",alignItems:"center",minWidth:88,flexShrink:0}}>
                            <div style={{fontSize:12,fontWeight:800,color:C.gold,lineHeight:1.15}}>
                              <MinuteDisplay raw={eventMinuteRaw(ev)} language={language} />
                            </div>
...
function SyncModal({ open, onClose, syncProfile, setSyncProfile, syncUid, saved, favTeams, setToast, setSaved, setFavTeams, dark, setDark, geoData, locationOverride, setLocationOverride, onShowSaved, userAvatar, persistAvatar, displayName, persistDisplayName, onSignOut=()=>{} }) {
  ...
  const homeContent = (
    <div style={{padding:isDesktop?"14px 24px 0":"16px 20px 0"}}>
      ...
      {/* Preferences */}
      <div style={{margin:"10px 0 10px",padding:"13px 14px",background:C.s2,border:`1px solid ${C.b1}`,borderRadius:12}}>
        <div style={{fontSize:11,color:C.mid,fontWeight:800,letterSpacing:"0.08em",marginBottom:10}}>⚙️ PREFERENCES</div>
        <WatchingFromSelector C={{...C, accent:C.green}} compact onChange={() => window.location.reload()} />
      </div>
...
export function QuickFacts({ tabTop }) {
  ...
  const { getScore } = useContext(LiveScoresCtx);
  const resolvedMatches = useResolvedTournamentMatches(getScore);
  ...
  // Use the same resolved knockout match source as Live/Schedule.
  // Before this, tournament facts were built from raw MATCHES, whose knockout rows
  // still contain placeholders like "R16 M1" / "TBD"; as a result, stats stopped at
  // the group stage even after Round of 32 results existed.
  const scores = resolvedMatches
    .map(m => {
      if (!fantasyConcreteTeam(m.home) || !fantasyConcreteTeam(m.away)) return null;
      let sc = getScore(m.home, m.away);
      let swapped = false;
      if (!sc) {
        sc = getScore(m.away, m.home);
        swapped = true;
      }
      if (!sc || !statusIsFinished(sc.status)) return null;
      const hg = Number(swapped ? sc.ag : sc.hg);
      const ag = Number(swapped ? sc.hg : sc.ag);
...
function AppContent() {
  ...
  const country = getBroadcastCountry(locationOverride?.country || geoData.country || "US");
